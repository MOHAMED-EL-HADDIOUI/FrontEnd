...
@Component({
  selector: 'app-deliberations-s1s1',
  templateUrl: './deliberations-s1s1.component.html',
  styleUrls: ['./deliberations-s1s1.component.css']
})
export class DeliberationsS1s1Component implements OnInit {
  status = 'empty';
  body: Body;
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('S1S1') !== null) {
      this.body = JSON.parse(localStorage.getItem('S1S1'));
  ...
  performCalculations() {
    ...
    localStorage.setItem('S1S1', JSON.stringify(this.body));
  }
  ...
  exportAsXls() {
    ...
    this.itemService.exportS1s1(main).pipe(
      catchError(err => {
  ...
  handle(event) {
    ...
    reader.onload = () => {
      ...
      this.itemService.postToImportS1s1(data).pipe(
        catchError(err => {
          return throwError(err);
        })
      ).subscribe(res => {
